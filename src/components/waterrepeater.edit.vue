  ...
  <div class="wrapper">
    <div class="wrapper-content">
        ...
        <div class="main-content autoheight" :style="autoheight">
          ...
          <div class="main-content-list">
            <div>
              ...
              <div>
                <div class="select-head" v-on:click.stop="dropDown('superiorMenu')">
                  <p>
                    <span>{{superior}}</span>
      ...
      <div class="relevance-area" v-if="showRelevanceArea"> 
        <div>
          <p>选择关联集中器</p>
          <p>请选择一个集中器设备，并点击下方确认键来添加关联集中器</p>
        </div>
        <div class="relevance-content meterlistheight" :style="meterlistheight">
          <div class="relevance-list" v-for="(waterConcentrator, index) in waterConcentratorList" :key="index"
          @click="selectMeter(waterConcentrator.id)">
            <div class="radioCheck" :class="{'radioChecked':relevanceID === waterConcentrator.id}">
              <!-- 选择框 -->
            </div>   
            <span class="relevance-list-code">{{waterConcentrator.communicationcode}}</span>
            <span class="relevance-list-name">{{waterConcentrator.site}}</span>
          </div>
        </div>
...
　　　 },
  ...
  mounted: function() {
    this.$ajax({
      ...
      .then(response => {
        ...
        this.ec = response.data[0].communicationcode
    ...
    this.$ajax({
      method: "post",
      url: "/waterconcentrator"
    })
      .then(response => {
        this.waterConcentratorList = response.data
      })
      .catch(error => {
        console.log(error)
      })
  ...
  methods: {
    ...
    save: function() {
      ...
      let superior
      (this.superior != '')?superior=this.superior:superior=this.superiorValue + " " + this.superiorName
      let supply
      ...
      // 为关联的水表修改relevance的值
      let repeaterRelevance = communicationcode + " " + site
      for(let i=0;i<this.waterCollectList.length;i++){
        // 如果水表relevance_id的值等于当前采集id
        if(this.waterCollectList[i].relevance_id == this.id){
          // 修改这个采集中的relevance的值
          this.$ajax({
            method: "post",
            url: "/watercollect/updata/relevance",
            data: {
              id: this.waterCollectList[i].id,
              relevance: repeaterRelevance
            }
          })
            .then(response => {
              console.log('s')
    ...
    chooseRelevanceMeter: function(id) {
      ...
      for(let i=0;i<this.waterConcentratorList.length;i++){
        if(this.waterConcentratorList[i].id == id){
          this.relevanceContent = this.waterConcentratorList[i].communicationcode + " " + this.waterConcentratorList[i].site
        }
      }
    },
    ...
    //       //   if(arr[i].communicationcode == this.ec){
    ...
    //       // }
    //       // if(arr == ""){
    //       //   this.superiorList = [{communicationcode:"无上级", site:""}]
    //       //   return
    //       // }else{
